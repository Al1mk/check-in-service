...
// newHandler returns a handler wired to a fresh store and a buffered jobs channel.
// The logger discards output so test output stays clean.
func newHandler() (*httpapi.EventHandler, chan forwarding.Job) {
	jobs := make(chan forwarding.Job, 10)
	h := httpapi.NewEventHandler(attendance.NewStore(), jobs, log.New(io.Discard, "", 0))
	return h, jobs
}
...
func TestPost_CheckOut_Returns200WithBody(t *testing.T) {
	...
	// A forwarding job must be enqueued immediately, without a worker running.
	select {
	case job := <-jobs:
		if job.EmployeeID != "E001" {
			t.Errorf("job.EmployeeID: want E001, got %q", job.EmployeeID)
		}
	default:
		t.Error("expected a forwarding job to be enqueued after check-out")
	}
}
